/*Program: Implementation of Banker's Algorithm*/
#include<stdio.h>
...
void main()
{
	...
	for(j = 0; j < nr; j++)
	{
		printf("No: of instances of Resource%d\t: ",j);
		scanf("%d",&Exist[j]);
	}
	...
	for(j = 0; j < nr; j++)
	{
		printf("Enter the maximum no: of R%d resources required by process%d\t",j,i);
		scanf("%d", &Max[i][j]);
	}
	...
	for(j = 0; j < nr; j++)
	{
		printf("Enter the current allocation: of R%d resources to the process%d\t",j,i);
